...
async function saveQuizAttempt(score, totalQuestions, answers, timeTaken) {
    try {
        const quizInfo = await getQuizzes(noteId);
        if (!quizInfo.success || !quizInfo.quiz) {
            console.error('Could not find quiz info for saving attempt');
            return false;
        }

        const response = await fetch('http://localhost:8080/BACKEND/saveQuizAttempt.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({
                quiz_id: quizInfo.quiz.id,
                answers: answers,
                score: score,
...
function finishQuiz() {
    ...
    const timeTaken = (timePerQuestion * quizData.length) - timeLeft; // approximate time taken

    // Save quiz attempt in background (don't block UI)
    saveQuizAttempt(results.correct, results.total, results.answers, timeTaken)
        .then(success => {
            if (!success) {
                console.warn('Quiz attempt could not be saved');
            }
        });
}
...
function calculateResults() {
    ...
    const results = {
        ...
        answers: [...userAnswers] // copy array
    };
